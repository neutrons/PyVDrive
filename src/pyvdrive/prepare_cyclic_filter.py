# This script is designed to execute in MantidPlot
import h5py
import numpy
from scipy.signal import argrelextrema


class CyclicDataSlicerProcess(object):
    
    def __init__(self):
        return

    @staticmethod
    def locate_cycle_boundaries(raw_ws_name, smoothed_ws_name, x_start, x_stop, cycle_local_max_lower_limit,
                                num_neighbors, trust_start_stop):
        """
        Locate boundaries of each cycle
        :param raw_ws_name:
        :param smoothed_ws_name:
        :param x_start:
        :param x_stop:
        :param cycle_local_max_lower_limit:
        :param num_neighbors:
        :param trust_start_stop:
        :return:
        """
...
#
def main_h5():
    tester = CyclicDataSlicerProcess()
...
def main_nxs():
    ...
    # Load data
    if not mtd.doesExist('raw_ws'):
        LoadEventNexus(Filename='/SNS/VULCAN/IPTS-22862/nexus/VULCAN_172282.nxs.h5', MetaDataOnly=True,
                       OutputWorkspace='raw_ws')

    # get logs
    raw_ws = mtd['raw_ws']
    furnace_log = raw_ws.run().getProperty('loadframe.furnace2')
    # vec_times = furnace_log.times
    vec_value = furnace_log.value
    vec_times = (furnace_log.times - furnace_log.times[0]).astype('float64') * 1.E-9
    
    # some magic number
    N = 5
    tester.pre_process_logs(vec_times, vec_value, N)
    ...
    num_neighbors = N
    trust_start_stop = True
    minima_indexes, maxima_indexes = tester.locate_cycle_boundaries(raw_ws_name, smoothed_ws_name, x_start, x_stop,
