...
import threading
from mantid.simpleapi import DiffractionFocussing, Load, GenerateEventsFilter, FilterEvents
from mantid.simpleapi import LoadDiffCal, AlignAndFocusPowder, Rebin, AlignDetectors, ConvertUnits
import mantid
import time
t0 = time.time()
...
print('{0}: Runtime = {1}   Total outputworkspace = {2}'.format(
    event_file_name, tf-t0, len(output_names)))
print('Details for thread = {3}:\n\tLoading  = {0}\n\tChopping = {1}\n\tFocusing = {2}'.format(
    t1-t0, t2-t1, tf-t2, num_threads))
