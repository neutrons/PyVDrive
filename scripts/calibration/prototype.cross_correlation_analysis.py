...
if 0:
    ...
    ConvertUnits(InputWorkspace='vulcan_diamond',
                 OutputWorkspace='vulcan_diamond', Target='dSpacing')
    Rebin(InputWorkspace='vulcan_diamond', OutputWorkspace='vulcan_diamond', Params='0.5,-0.0003,3')
    ConvertToMatrixWorkspace(InputWorkspace='vulcan_diamond',
                             OutputWorkspace='vulcan_diamond_matrix')
...
if True:
    ...
    calib_ws_1fit_dict, mask_ws_1fit_dict =\
        cross_correlation.cross_correlate_vulcan_data(
            diamond_ws_name, group_ws_name, fit_time=1, flag='1fit')

    # do cross-correlation with 2 fit
    calib_ws_2fit_dict, mask_ws_2fit_dict =\
        cross_correlation.cross_correlate_vulcan_data(
            diamond_ws_name, group_ws_name, fit_time=2, flag='2fit')
    ...
    # do cross-correlation with 1 fit
    calib_ws_1fit_dict, mask_ws_1fit_dict =\
        cross_correlation.cross_correlate_vulcan_data_2bank(
            diamond_ws_name, group_ws_name, fit_time=1, flag='1fit')
...
for bank_name in bank_name_list:
    ...
    print('Masked pixels set difference: {0} vs {1}'.format(
        len(diff_spectra_set), len(diff_spectra_set_op)))
