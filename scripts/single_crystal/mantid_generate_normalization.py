...
van_bkgd_ws = LoadEventNexus(Filename=bkgd_nexus, OutputWorkspace='VAN_BKGD_{}'.format(background_run_number))
...
van_count_clean_name = van_count_clean
...
if True:
    ...
    van_bkgd_ws = Rebin(InputWorkspace=van_bkgd_ws, OutputWorkspace=van_bkgd_ws_name)
    van_bkgd_bank3 = SumSpectra(InputWorkspace=van_bkgd_ws_name,
                                OutputWorkspace='Bank3_{}'.format(bkgd_rub_number),
                                StartWorkspaceIndex=6467)
...
smooth_van_bank3 *= 1./van_count_clean

# [Save for result]
import h5py

norm_file = h5py.File('vanadium_{}_norm.hdf5', 'w')
...
smooth_group['note'] = 'with background {} removed; normalized by total counts'.format(bkgd_run_number)

count_group = norm_file.create_group('counts')
count_group['note'] = 'with background {} removed'.format(bkgd_run_number)
van_count_clean = Transpose(van_count_clea)
count_group.create_dataset('counts', data=van_count_clean)
