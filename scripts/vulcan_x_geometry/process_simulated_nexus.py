...
ws_index_range = {'B1': (0, 20*8*512),  'B2': (20*8*512, 2 * 20*8*512),
                  'B5': (2 * 20*8*512,  2*20*8*512+9*8*256)}

# -----------------------------   REDUCE -------------------------------
if False:
    # reduce
    diamond_ws = Load(Filename=nexus_file_name, OutputWorkspace=diamond_ws_name)
    diamond_ws = ConvertUnits(InputWorkspace=diamond_ws,
                              OutputWorkspace=diamond_ws, Target='dSpacing', EMode='Elastic')
    diamond_ws = Rebin(InputWorkspace=diamond_ws_name,
                       OutputWorkspace=diamond_ws_name, Params='0.3, -0.0003, 3.0')
...
if False:
    ...
    for bank in sorted(ws_index_range.keys()):
        ...
        # END-FOR
        print('{} ({}, {}): {}'.format(
            bank, ws_index_range[bank][0], ws_index_range[bank][1], num_events))
    # END-FOR
# END-IF
...
if True:
    # focus
    for bank in sorted(ws_index_range.keys()):
        SumSpectra(InputWorkspace=diamond_ws_name,
                   StartWorkspaceIndex=ws_index_range[bank][0], EndWorkspaceIndex=ws_index_range[bank][1]-1, OutputWorkspace=bank)
    # END-FOR
