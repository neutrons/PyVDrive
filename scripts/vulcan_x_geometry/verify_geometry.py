    ...
    for bank_id in sorted(Bank_Parameter_Dict[Current_Phase].keys()):
        ...
        if (num_tubes/8) % 2 == 0:
            ...
            center_ws_index_dict['upper left'] = \
                (8 * num_tube_pixels) * left_pack_id + (num_tube_pixels * 7) + num_tube_pixels / 2
            center_ws_index_dict['lower left'] = center_ws_index_dict['upper left'] - 1

            center_ws_index_dict['upper right'] = (8 * num_tube_pixels) * right_pack_id + num_tube_pixels / 2
            center_ws_index_dict['lower right'] = center_ws_index_dict['upper right'] - 1

        else:
            # odd: pick the middle pack
            middle_pack_id = (num_tubes/8) / 2
            start_pack_index = middle_pack_id * 8

            # find center
            center_ws_index_dict['upper left'] = start_pack_index + 3 * num_tube_pixels + num_tube_pixels / 2
            center_ws_index_dict['lower left'] = center_ws_index_dict['upper left'] - 1
        ...
        for pos_name in corner_ws_index_dict:
            ...
            detector_id_list.append(ws_index_i)
        MaskDetectorList(InputWorkspace=ws_name, OutputWorkspace=ws_name, DetectorIDList=detector_id_list)
